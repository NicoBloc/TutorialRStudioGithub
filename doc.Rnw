...
boxplot(x, main='Boring boxplot')
hist(x, main='Boring histogram')
@

Cool plot (Brownian motion):

<<cool-plots, fig.width=6, fig.height=4, out.width='.4\\linewidth'>>=
plot(cumsum(x), type='l')
silent <- lapply(2:4, function(i) lines(cumsum(sample(x)), col=i))
@
